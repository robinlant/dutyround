...
var (
	...
	ErrInvalidMaxParticipants = errors.New("max participants must be at least 1")
)
...
func (s *OccurrenceService) CreateOccurrence(ctx context.Context, o domain.Occurrence) (domain.Occurrence, error) {
	if o.MaxParticipants < 1 {
		return domain.Occurrence{}, ErrInvalidMaxParticipants
	}
...
// Withdraw removes a user's own participation. Returns ErrNotSignedUp if the user has no participation.
func (s *OccurrenceService) Withdraw(ctx context.Context, occurrenceID, userID int64) error {
	parts, err := s.participations.FindByOccurrence(ctx, occurrenceID)
	if err != nil {
		return err
	}
	found := false
